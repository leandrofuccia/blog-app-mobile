...
export default function DrawerNavigator() {
  ...
  return (
    <Drawer.Navigator
      initialRouteName="Início"
      screenOptions={{
        drawerActiveTintColor: colors.destaque,
        drawerInactiveTintColor: colors.textoSecundario,
        drawerActiveBackgroundColor: '#D1FAE5', // Verde clarinho
        headerStyle: {
      ...
      drawerContent={(props: { state: { routes: any[]; index: any; }; descriptors: { [x: string]: { options: {
        ...
        return (
          <DrawerContentScrollView {...props}
            style={{ backgroundColor: colors.neutroFundo }}
          >
            <View style={{ padding: 16 }}>
              <Text
                style={{
            ...
            {inicioRoutes.map((route: { key: string | number; name: any; }) => (
              <DrawerItem
                key={route.key}
                label={props.descriptors[route.key].options.drawerLabel ?? route.name}
                icon={props.descriptors[route.key].options.drawerIcon}
            ...
            {adminRoutes.map((route: { key: string | number; name: any; }) => (
              <DrawerItem
                ...
                focused={
                  props.state.index ===
                  props.state.routes.findIndex((r: { key: string | number; }) => r.key === route.key)
                }
                onPress={() => props.navigation.navigate(route.name)}
              />
            ))}

            {outrosRoutes.map((route) => (
              <DrawerItem
                key={route.key}
                label={props.descriptors[route.key].options.drawerLabel ?? route.name}
                icon={props.descriptors[route.key].options.drawerIcon}
